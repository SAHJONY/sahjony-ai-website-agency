  ...
  for (let number = 1; number <= maxAttempts; number++) {
    ...
    try {
      ...
      try {
        ...
        providerCode = clean(payload && (payload.name || payload.code || payload.error), 80).replace(/[^a-zA-Z0-9_.-]/g, "_");
      } catch (_) {}
      lastError = status ? `provider_http_${status}${providerCode ? `_${providerCode}` : ""}` : "provider_rejected";
