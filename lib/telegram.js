...
export async function tgHandleUpdate(update, baseUrl) {
  ...
  if (owner && String(chatId) === String(owner)) {
    ...
    const pay = text.match(/^\/pay\s+(\d{4,})\s+(\d+(?:\.\d+)?)(?:\s+(\d+(?:\.\d+)?))?(?:\s+(\d+))?(?:\s+(\d+(?:\.\d+)?))?/i);
    if (pay) {
      const [, cid, b, mo, inst, dn] = pay;
      let j = null;
      try {
        const r = await fetch(baseUrl + "/api/checkout", {
          method: "POST",
          headers: { "content-type": "application/json", "x-admin-token": process.env.ADMIN_PASSWORD || "" },
          body: JSON.stringify({ name: "Client", build: Number(b) || 0, monthly: Number(mo) || 0, installments: Number(inst) || 1, downPayment: Number(dn) || 0 }),
        });
        j = await r.json();
      } catch (_) {}
