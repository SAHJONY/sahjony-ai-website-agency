...
const AUTH_CHECK_URL = "https://www.sahjony.com/api/connect/worldwide/search";
const FIELD_MASK = [
...
export async function worldwidePlacesProxy(req, res) {
  ...
  try {
    const auth = await fetch(AUTH_CHECK_URL, {
      ...
      body: JSON.stringify({ query: "owner auth check", limit: 1, language_code: "en", region_code: "US" }),
      signal: AbortSignal.timeout(8000),
    });
    // Current Trading OS returns 503 after successful owner validation because
    // its local Places key is absent. Treat 503 as authenticated; 401/403 are not.
    if (!(auth.ok || auth.status === 503 || auth.status === 502)) {
      return res.status(auth.status === 403 ? 403 : 401).json({ error: "Owner authorization failed" });
    }
  } catch {
